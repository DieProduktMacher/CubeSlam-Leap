...
var lc,
    listen,
    leapData = [],
    keys = ['x', 'y', 'z', 'pitch', 'yaw'],
    NORMAL_SPEED = 50, SPEED_TOLERANCE = 20,
    NORMAL_Z = 200, Z_LOWER = 60, Z_UPPER = 400, Z_RANGE = 50,
    NORMAL_ZOOM = 1, ZOOM_TOLERANCE = 0.05, ZOOM_LOWER = .92, ZOOM_HIGHER = 1.08, 
    NORMAL_PITCH = 0, PITCH_TOLERANCE = .2, PITCH_LOWER = -.60, PITCH_HIGHER = .70,
    NORMAL_YAW = 0, YAW_TOLERANCE = 0.2, YAW_LOWER = -.75, YAW_HIGHER = .75;
...
exports.enter = function(context){
  ...
  lc = new Leap.Controller("ws://localhost:6437/");
  listen = new Leap.Listener();

  lc.addListener(listen);

  listen.onFrame = function(e){
      var frame = e.frame(),
          movement = {};
      if(frame !== undefined){
        ...
        if(frame.hands().length > 0){
          ...
          var data = frame.hands()[0];

          // pan - x,y
          movement.x = data.palmPosition().x;
          movement.y = data.palmPosition().z;

          //zoom - z
          movement.z = data.palmPosition().y;

          //pitch - pitch
          var normal = data.palmNormal();
          movement.pitch = -1 * normal.z; // leap motion has it that negative is sloping upwards, flipping it for google earth
          //Math.atan2(normal.z, normal.y) * 180/math.pi + 180;

          //yaw - yaw
          movement.yaw = -1 * normal.x; // roll?
      ...
      /* Platformer controls start */

      // movement x axis
      //console.log(lastTime);
      //console.log(Math.floor((new Date() - lastTime)));
      //if ( Math.floor((new Date() - lastTime)) > 250 ) {
        //console.log('sd');
      ...
      /* Platformer controls end */


      /* Flight controls start */
      ...
      if(Math.floor(d - lastTime) > 10){
        console.log(movement.x );
        
        if(movement.x !== undefined){
          if(movement.x > parseFloat(50)){
            if (lastmove != movement.x) {
              onright();
              lastTime =  Date.now();
              lastmove = movement.x;
            }
          } else if (movement.x < parseFloat(-50)){
            if (lastmove != movement.x) {
              onleft();
              lastTime =  Date.now();
              lastmove = movement.x;
        ...
        // movement x axis
        if(mv.yaw !== undefined) {
           if(mv.yaw > parseFloat(0.2)){
              onright();
              lastTime =  Date.now();
           } else if(mv.yaw < parseFloat(-0.2)){
             onleft();
             lastTime =  Date.now();
           }
        }
